            ...
            <div class="tab-pane {{ !Request('active') || Request('active') == CashExpense::PAYABLE_CHEQUE ?'active':'' }}" id="{{ CashExpense::PAYABLE_CHEQUE }}" role="tabpanel">
                <div class="kt-portlet kt-portlet--mobile">
                    <x-table-title.with-two-dates :type="CashExpense::PAYABLE_CHEQUE" :title="__('Payable Cheques')" :startDate="$filterDates[CashExpense::PAYABLE_CHEQUE]['startDate']??''" :endDate="$filterDates[CashExpense::PAYABLE_CHEQUE]['endDate']??''">
                        <x-export-cash-expense :route-redirect="route('view.cash.expense',['company'=>$company->id])" :route-action="route('payable.cheque.mark.as.paid',['company'=>$company->id])" :popup-title="__('Do You Want To Mark This Cheque / Cheques As Paid ?')" :account-types="$accountTypes" :financialInstitutionBanks="$financialInstitutionBanks" :search-fields="$payableChequesTableSearchFields" :cash-expense-type="CashExpense::PAYABLE_CHEQUE" :has-search="1" :has-batch-collection="1" :banks="$banks??[]" :selectedBanks="$selectedBanks" href="{{route('create.cash.expense',['company'=>$company->id])}}" />
            ...
            <div class="tab-pane {{ Request('active') == CashExpense::OUTGOING_TRANSFER ? 'active':''  }}" id="{{ CashExpense::OUTGOING_TRANSFER }}" role="tabpanel">
                <div class="kt-portlet kt-portlet--mobile">
                    ...
                    <x-table-title.with-two-dates :type="CashExpense::OUTGOING_TRANSFER" :title="__('Outgoing Transfer')" :startDate="$filterDates[CashExpense::OUTGOING_TRANSFER]['startDate']??''" :endDate="$filterDates[CashExpense::OUTGOING_TRANSFER]['endDate']??''">
                        <x-export-cash-expense :route-redirect="route('view.cash.expense',['company'=>$company->id])" :route-action="route('outgoing.transfer.mark.as.paid',['company'=>$company->id])" :popup-title="__('Do You Want To Mark This Outcoming Transfer/s As Paid ?')"  :account-types="$accountTypes" :financialInstitutionBanks="$financialInstitutionBanks" :search-fields="$outgoingTransferTableSearchFields" :cash-expense-type="CashExpense::OUTGOING_TRANSFER" :has-search="1" :has-batch-collection="1" :banks="$banks??[]" :selectedBanks="$selectedBanks" href="{{route('create.cash.expense',['company'=>$company->id])}}" />
