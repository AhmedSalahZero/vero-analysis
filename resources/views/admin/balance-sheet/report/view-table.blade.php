...
<x-tables.basic-view  :form-id="'store-report-form-id'" :wrap-with-form="true" :form-action="route('admin.store.balance.sheet.report',['company'=>getCurrentCompanyId()])"  class="position-relative table-with-two-subrows main-table-class" id="{{ $tableId }}">
    <x-slot name="filter">
                        ...
                        render: function(d, b, row) {
                            ...
                            if(! row.isSubItem && row.has_sub_items){
                                ...
                                if(row.sub_items.length){
                                    // elements += `<a data-is-subitem="0" data-balance-sheet-item-id="${row.id}" data-balance-sheet-id="${modelId}" class="d-block  text-danger" href="#" data-toggle="modal" data-target="#delete-all-sub-modal${row.id}" >{{ __('Delete') }}</a> `
                                }
                                return elements ;
                            }
                            else if(row.isSubItem){
                                return `<a data-is-subitem="1" class="d-block edit-btn mb-2 text-white " href="#" data-toggle="modal" data-is-depreciation-or-amortization="${row.pivot.is_depreciation_or_amortization}" data-balance-sheet-id="${row.pivot.balance_sheet_id}" data-target="#edit-sub-modal${row.pivot.balance_sheet_item_id + row.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }"> <i class="fa fa-pen-alt"></i>  </a> <a class="d-block  delete-btn text-white mb-2 text-danger" href="#" data-toggle="modal" data-target="#delete-sub-modal${row.pivot.balance_sheet_item_id + row.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }">
		...
		table.DataTable(
            {
                ...
                , createdRow: function(row, data, dataIndex, cells,x) {
                    ...
                    var balanceSheetId= data.isSubItem ? data.pivot.balance_sheet_id  : $('#model-id').val() ;
                    var balanceSheetItemId = data.isSubItem ? data.pivot.balance_sheet_item_id  : data.id ;
            ...
            <label>{{ __('Is Depreciation Or Amortization ? ') }}</label>
                    ...
                    <div class="modal fade" id="edit-sub-modal${data.pivot.balance_sheet_item_id + data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-')}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content modal-lg">
      ...
      <div class="modal-body">
        <form id="edit-sub-item-form${data.pivot.balance_sheet_item_id + data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-')  }" class="edit-submit-sub-item" action="{{ route('admin.update.balance.sheet.report',['company'=>getCurrentCompanyId()]) }}">
            
            <input type="hidden" name="balance_sheet_item_id"  value="${data.pivot.balance_sheet_item_id}">
            <input  type="hidden" name="balance_sheet_id"  value="{{ $balanceSheet->id }}">
           ...
           <div class="mt-2">
            ...
            <select name="sub_of_id" class="form-control main-row-select" data-selected-main-row="${data.pivot.balance_sheet_item_id}">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary save-sub-item-edit" data-id="${data.pivot.balance_sheet_item_id}" data-sub-item-name="${data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }">{{ __('Edit') }}</button>
      </div>
    </div>
  </div>
    </div>
    `
                    ...
                    <div class="modal fade" id="delete-sub-modal${data.pivot.balance_sheet_item_id + data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-')}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content modal-lg">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <form id="delete-sub-item-form${data.pivot.balance_sheet_item_id+data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }" class="delete-submit-sub-item" action="{{ route('admin.destroy.balance.sheet.report',['company'=>getCurrentCompanyId()]) }}">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('Close')  }}</button>
        <button type="button" class="btn btn-primary save-sub-item-delete" data-id="${data.pivot.balance_sheet_item_id}" data-sub-item-name="${data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }" >{{ __('Delete') }}</button>
      </div>
    </div>
  </div>
    </div>
    `
                    ...
                    $(cells).filter('.editable.editable-text').each(function(index,textDt){
                           ...
                           var hiddenInput = `<input type="hidden" class="text-input-hidden"  name="balanceSheetItemName[${balanceSheetId}][${balanceSheetItemId}][${subItemName}]" value="${$(textDt).html()}" > `;
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content modal-lg">
      ...
      <div class="modal-body">
        <form id="add-sub-item-form${data.id}" class="submit-sub-item" action="{{ route('admin.store.balance.sheet.report',['company'=>getCurrentCompanyId()]) }}">
            
            <label class="label ">{{ __('How Many Items ?') }}</label>
            <input type="hidden" name="balance_sheet_item_id"  value="${data.id}">
            <input  type="hidden" name="balance_sheet_id"  value="{{ $balanceSheet->id }}">
