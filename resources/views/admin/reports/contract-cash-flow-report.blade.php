				...
				@if($reportInterval == 'weekly')
					...
					@endif
                            ...
                            <tbody>
                                ...
                                @endphp
								{{-- for customer  --}}
                                {{-- @foreach(array_merge(['Cash & Banks Begining Balance','Checks Collected','Incoming Transfers','Bank Deposits','Cash Collections','Customers Invoices','Customers Past Due Invoices','Cheques In Safe','Cheques Under Collection','Sales Forecast Collections','Total Cash Inflow'
								]) as $customerName)
                                ...
                                @endphp
                               @include('admin.reports.cash-flow-main-row')
								...
								{{-- for suppliers  --}}
								 ...
								 @foreach( $result[$mainReportKey] ?? [] as $parentKeyName => $subRows)
								 @php
									...
									|| $parentKeyName == __('Net Cash (+/-)') || $parentKeyName == __('Total Cash Inflow') || $parentKeyName == __('Total Cash Outflow')
                           		     ...
                           		     $rowIndex = $rowIndex+ 1;
										$subRowKeys = HArr::removeKeyFromArrayByValue(array_keys($result[$mainReportKey][$parentKeyName]),['total']);
										
								 @endphp
								  @include('admin.reports.cash-flow-main-row',['isTotalRow'=>true ])
								...
								{{-- @foreach(['Outgoing Transfers','Cash Payments','Paid Payable Cheques','Under Payment Payable Cheques','Suppliers Invoices'] as $currentSupplierKeyName) --}}
                                        @include('admin.reports.cash-flow-sub-row')
                                @endforeach
								@endforeach 
