...
<x-tables.basic-view    class="position-relative  main-table-class" id="{{ $tableId }}">
    <x-slot name="filter">
        @include('admin.cash-flow-statement.filter' , [
            'type'=>'filter'
        ])
    </x-slot>

    <x-slot name="export">
        @include('admin.cash-flow-statement.export' , [
            'type'=>'export'
        ])
    </x-slot>
		...
		table.DataTable(
            {
                ...
                , "columns": [
                    ...
                    , {
                        ...
                        <a href="/`+$('body').data('lang')+'/'+ $('body').data('current-company-id') +`/cash-flow-statement/${row.id}/report"  data-id="${row.id}" data-model-name="{{$modelName}}" class="btn btn-sm cursor-pointer btn-clean btn-icon btn-icon-md" title="{{ __('Edit Balance Sheet Report') }}">
