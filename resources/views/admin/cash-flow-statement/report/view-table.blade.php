...
<div class="table-custom-container position-relative  ">
    ...
    <input type="hidden" id="sales-growth-rate-id" value="{{ \App\Models\CashFlowStatementItem::SALES_GROWTH_RATE_ID }}">
    <input type="hidden" id="sales-revenue-id" value="{{ \App\Models\CashFlowStatementItem::SALES_REVENUE_ID }}">
    <input type="hidden" id="gross-profit-id" value="{{ \App\Models\CashFlowStatementItem::GROSS_PROFIT_ID }}">
    <input type="hidden" id="market-expenses-id" value="{{ \App\Models\CashFlowStatementItem::MARKET_EXPENSES_ID }}">
    <input type="hidden" id="sales-and-distribution-expenses-id" value="{{ \App\Models\CashFlowStatementItem::SALES_AND_DISTRIBUTION_EXPENSES_ID }}">
    <input type="hidden" id="general-expenses-id" value="{{ \App\Models\CashFlowStatementItem::GENERAL_EXPENSES_ID }}">
    <input type="hidden" id="earning-before-interest-taxes-depreciation-amortization-id" value="{{ \App\Models\CashFlowStatementItem::EARNING_BEFORE_INTEREST_TAXES_DEPRECIATION_AMORTIZATION_ID }}">
    <input type="hidden" id="earning-before-interest-taxes-id" value="{{ \App\Models\CashFlowStatementItem::EARNING_BEFORE_INTEREST_TAXES_ID }}">
    ...
    <x-tables.basic-view :redirect-route="route('admin.view.financial.statement', getCurrentCompanyId())" :save-and-return="true" :form-id="'store-report-form-id'" :wrap-with-form="true" :form-action="route('admin.store.cash.flow.statement.report',['company'=>getCurrentCompanyId()])" class="position-relative table-with-two-subrows main-table-class" id="{{ $tableId }}">
        <x-slot name="filter">
            @include('admin.cash-flow-statement.report.filter' , [
            'type'=>'filter'
            ])
        </x-slot>

        <x-slot name="export">
            @include('admin.cash-flow-statement.report.export' , [
            'type'=>'export'
            ])
        </x-slot>
                ...
                window.addEventListener('DOMContentLoaded', function() {
                    (function($) {
                        ...
                        function formateTableForNewRow(formDataObject) {
                            ...
                            let cashFlowStatementId = formDataObject.financial_statement_able_id;

                            let cashFlowStatementItemId = formDataObject.financial_statement_able_item_id;
                            let salesRevenueId = domElements.salesRevenueId;
                            rows = ``;
                            var i = 0;
                            for (i; i < numberOfAddedItems; i++) {
                                ...
                                rows += getRowForSubItemsTr('kt_table_1', dates, cashFlowStatementId, cashFlowStatementItemId, subItemName, isDepreciationOrAmortization, isQuantity, canBePercentage, percentageOrFixed, isPercentage, isRepeatingFixed, isNoneRepeatingFixed, valuesOfDates, canTriggerChange, isPercentageOf);
                            }
                        ...
                        function getRowForSubItemsTr(tableId, dates, cashFlowStatementId, cashFlowStatementItemId, subItemName, isDepreciationOrAmortization, isQuantity, canBePercentageOrFixed, fixedOrPercentage, isPercentage, isRepeatingFixed, isNoneRepeatingFixed, valuesOfDates, canTriggerChange, isPercentageOf) {
                            let row = `<tr  data-financial-statement-able-item-id="${cashFlowStatementItemId}" class="d-none edit-info-row add-sub maintable-1-row-class${cashFlowStatementItemId} is-sub-row even" data-sub-item-name="${subItemName}" data-is-trigger-change="${canTriggerChange}" data-can-be-percentage-or-fixed="${canBePercentageOrFixed}" data-percentage-or-fixed="${fixedOrPercentage}" data-is-percentage="${isPercentage}" data-is-percentage-of="${isPercentageOf}"  data-is-repeating-fixed="${isRepeatingFixed}" data-is-none-repeating-fixed="${isNoneRepeatingFixed}">
											...
											<td class="sub-text-bg text-nowrap editable editable-text is-name-cell dtfc-fixed-left" data-cash-flow-statement-id="${cashFlowStatementId}" data-main-model-id="${cashFlowStatementItemId}" data-cash-flow-statement-item-id="${cashFlowStatementItemId}" data-main-row-id="${cashFlowStatementItemId}" data-sub-item-name="${subItemName}" data-table-id="${tableId}" data-is-quantity="${isQuantity}" style="left: 141.417px; position: sticky;" contenteditable="true">${subItemName}</td><input type="hidden" class="text-input-hidden" name="financialStatementAbleItemName[${cashFlowStatementId}][${cashFlowStatementItemId}][${subItemName}]" value="${subItemName}">`;
                            dates.forEach(function(date) {
                                ...
                                row += `<td class="sub-numeric-bg text-nowrap editable editable-date date-${date}" data-cash-flow-statement-id="${cashFlowStatementId}" data-main-model-id="${cashFlowStatementId}" data-cash-flow-statement-item-id="${cashFlowStatementItemId}" data-main-row-id="${cashFlowStatementItemId}" data-sub-item-name="${subItemName}" data-table-id="${tableId}" data-is-quantity="${isQuantity}" contenteditable="true">${currentValue}</td>

								<input type="hidden" data-sub-item-name="${subItemName}" data-cash-flow-statement-id="${cashFlowStatementId}" data-cash-flow-statement-item-id="${cashFlowStatementItemId}" name="value[${cashFlowStatementId}][${cashFlowStatementItemId}][${subItemName}][${date}]" data-date="${date}" data-parent-model-id="${cashFlowStatementItemId}" value="0">`;
                            })

                            row += `<td class="  sub-numeric-bg text-nowrap total-row">0</td>
										<input type="hidden" class="input-hidden-for-total" name="subTotals[${cashFlowStatementId}][${cashFlowStatementItemId}][${subItemName}]" data-parent-model-id="${cashFlowStatementItemId}" value="0">
									</tr>`;
                        ...
                        function getInputOfTdElement(tdElement, mainRowId, date) {
                            ...
                            const cashFlowStatementItemId = tdElement.getAttribute('data-main-row-id')
                            return document.querySelector('input[data-cash-flow-statement-item-id="' + cashFlowStatementItemId + '"][data-sub-item-name="' + subItemName + '"][data-date="' + date + '"]');
                        }
                        ...
                        function getMainRowOfSubItemTd(tdElement) {
                            const cashFlowStatementItemId = tdElement.getAttribute('data-cash-flow-statement-item-id')
                            const mainRowOfSubItem = document.querySelector('.maintable-1-row-class' + cashFlowStatementItemId)
                        ...
                        function getDates() {
                            var dates = "{{ json_encode(array_keys($cashFlowStatement->getIntervalFormatted())) }}";
                                ...
                                function() {
                                    ...
                                    columns.push({
                                        render: function(d, b, row) {
                                            ...
                                            if (!row.isSubItem && row.has_sub_items) {
                                                elements = `<a data-is-subitem="0" data-cash-flow-statement-item-id="${row.id}" data-cash-flow-statement-id="${modelId}" class="d-block add-btn mb-2" href="#" data-toggle="modal" data-target="#add-sub-modal${row.id}">{{ __('Add') }}</a> `;
                                                if (row.sub_items.length) {}
                                                return elements;
                                            } else if (row.isSubItem && (row.pivot.created_from == row.pivot.sub_item_type)) {
                                                return `
											<div class="d-flex align-items-center justify-content-between">
												<a  data-is-subitem="1" data-cash-flow-statement-item-id="${row.pivot.financial_statement_able_item_id}" data-cash-flow-statement-id="${row.pivot.financial_statement_able_id}" class="d-block edit-btn mb-2 text-white " href="#" data-toggle="modal" data-is-depreciation-or-amortization="${row.pivot.is_depreciation_or_amortization}" data-cash-flow-statement-id="${row.pivot.financial_statement_able_id}" data-target="#edit-sub-modal${row.pivot.financial_statement_able_item_id + row.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }"> <i class="fa fa-pen-alt"></i>  </a> <a data-cash-flow-statement-item-id="${row.pivot.financial_statement_able_item_id}" data-cash-flow-statement-id="${row.pivot.financial_statement_able_id}" class="d-block  delete-btn text-white mb-2 text-danger" href="#" data-toggle="modal" data-target="#delete-sub-modal${row.pivot.financial_statement_able_item_id + row.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }">
													<i class="fas fa-trash-alt"></i>
                                    ...
                                    table.DataTable({
                                            ...
                                            , createdRow: function(row, data, dataIndex, cells) {
                                                ...
                                                var cashFlowStatementId = data.isSubItem ? data.pivot.financial_statement_able_id : $('#model-id').val();
                                                var cashFlowStatementItemId = data.isSubItem ? data.pivot.financial_statement_able_item_id : data.id;
                                                ...
                                                $(cells).filter(".editable").attr('contenteditable', true)
                                                    .attr('data-cash-flow-statement-id', cashFlowStatementId)
                                                    .attr('data-main-model-id', cashFlowStatementId)
                                                    .attr('data-cash-flow-statement-item-id', cashFlowStatementItemId)
                                                    .attr('data-main-row-id', cashFlowStatementItemId)
									...
									<div class="modal fade" id="edit-sub-modal${data.pivot.financial_statement_able_item_id + data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-')}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
											<div class="modal-dialog" role="document">
												...
												<div class="modal-body">
													<form data-financial-statement-able-item-id="${data.pivot.financial_statement_able_item_id}" id="edit-sub-item-form${data.pivot.financial_statement_able_item_id + data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-')  }" class="edit-submit-sub-item" action="{{ route('admin.update.cash.flow.statement.report',['company'=>getCurrentCompanyId()]) }}">
														<input type="hidden" name="sub_item_type" value="{{ getReportNameFromRouteName(Request()->route()->getName()) }}">
											...
											<div class="modal fade" id="delete-sub-modal${data.pivot.financial_statement_able_item_id + data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-')}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
													<div class="modal-dialog" role="document">
														...
														<div class="modal-body">
															<form id="delete-sub-item-form${data.pivot.financial_statement_able_item_id+data.pivot.sub_item_name.replaceAll('/','-').replaceAll('&','-').replaceAll('%','-').replaceAll(' ','-').replaceAll('(','-').replaceAll(')','-') }" class="delete-submit-sub-item" action="{{ route('admin.destroy.cash.flow.statement.report',['company'=>getCurrentCompanyId()]) }}">
																<input type="hidden" name="sub_item_type" value="{{ getReportNameFromRouteName(Request()->route()->getName()) }}">
                                                    ...
                                                    $(row).addClass('edit-info-row').addClass('add-sub maintable-1-row-class' + (cashFlowStatementItemId))
                                                    ...
                                                    if (data.pivot.can_be_percentage_or_fixed) {
                                                        ...
                                                        $(row).attr('data-financial-statement-able-item-id', cashFlowStatementItemId)
                                                    ...
                                                    $(cells).filter('.editable.editable-date').each(function(index, dateDt) {
                                                        ...
                                                        var hiddenInput = `<input data-sub-item-name="${data.pivot?data.pivot.sub_item_name:''}" data-cash-flow-statement-id="${cashFlowStatementId}" data-cash-flow-statement-item-id="${cashFlowStatementItemId}" type="hidden" name="value[${cashFlowStatementId}][${cashFlowStatementItemId}][${subItemName}][${filterDate}]" data-date="${filterDate}" data-parent-model-id="${cashFlowStatementItemId}" value="${($(dateDt).html().replace(/(<([^>]+)>)/gi, "").replace(/,/g, ""))}" > `;
                                                        $(dateDt).after(hiddenInput);

                                                    });

                                                    $(row).append(
                                                        `<input data-sub-item-name="${data.pivot?data.pivot.sub_item_name:''}" type="hidden" class="input-hidden-for-total" name="subTotals[${cashFlowStatementId}][${cashFlowStatementItemId}][${subItemName}]"  data-parent-model-id="${cashFlowStatementItemId}" value="0" >`
                                                    );
                                                    ...
                                                    $(cells).filter('.editable.editable-text').each(function(index, textDt) {
                                                        ...
                                                        var hiddenInput = `<input type="hidden" class="text-input-hidden"  name="financialStatementAbleItemName[${cashFlowStatementId}][${cashFlowStatementItemId}][${subItemName}]" value="${$(textDt).html()}" > `;
                                                    ...
                                                    if (!data.has_sub_items) {
                                                        ...
                                                        $(cells).filter('.editable.editable-date').each(function(index, dateDt) {
                                                            ...
                                                            var hiddenInput = `<input type="hidden" name="valueMainRowWithoutSubItems[${cashFlowStatementId}][${cashFlowStatementItemId}][${filterDate}]" data-date="${filterDate}" data-parent-model-id="${cashFlowStatementItemId}" value="${($(dateDt).html().replace(/(<([^>]+)>)/gi, "").replace(/,/g, ""))}" > `;
                                                            $(dateDt).after(hiddenInput);
                                                        ...
                                                        $(row).append(`
											<input type="hidden" class="input-hidden-for-total" name="totals[${cashFlowStatementId}][${cashFlowStatementItemId}]" value="0">
										`);
                                                        ...
                                                        $(cells).filter('.editable.editable-date').each(function(index, dateDt) {
                                                            ...
                                                            var hiddenInput = `<input type="hidden" class="main-row-that-has-sub-class" name="valueMainRowThatHasSubItems[${cashFlowStatementId}][${cashFlowStatementItemId}][${filterDate}]" data-date="${filterDate}" data-parent-model-id="${cashFlowStatementItemId}" value="${($(dateDt).html().replace(/(<([^>]+)>)/gi, "").replace(/,/g, ""))}" > `;
                                                            $(dateDt).after(hiddenInput);
                                                        });
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content modal-xl">
      ...
      <div class="modal-body">
        <form  data-financial-statement-able-item-id="${data.id}" id="add-sub-item-form${data.id}" class="submit-sub-item" action="{{ route('admin.store.cash.flow.statement.report',['company'=>getCurrentCompanyId()]) }}">
      ...
      <div class="modal-footer" style="border-top:0 !important">
        ...
        <button type="button" class="btn btn-primary save-sub-item" data-redirect-to='' data-id="${data.id}">{{ __('Save') }}</button>
                    ...
                    `)
                                            ...
                                            , drawCallback: function(settings) {
                                                ...
                                                var addBtnForCorporateTaxes = document.querySelector('.add-btn[data-cash-flow-statement-item-id="' + corporateTaxesId + '"]')
                                                var deleteBtnForCorporateTaxes = document.querySelector('.delete-btn[data-cash-flow-statement-item-id="' + corporateTaxesId + '"]')
        ...
        chart.cursor = new am4charts.XYCursor();
                        ...
                        jQuery(document).ready(function() {
                            ...
                            $(document).on('click', '.save-sub-item-edit', function(e) {
                                ...
                                if (formDataObject['sub_items[0][can_be_percentage_or_fixed]']) {
                                    var cashFlowStatementId = formDataObject['financial_statement_able_id'];
                                    var cashFlowStatementItemId = formDataObject['financial_statement_able_item_id'];
                                    ...
                                    $('tr.maintable-1-row-class' + cashFlowStatementItemId + '[data-sub-item-name="' + oldSubItemName + '"]').attr('data-financial-statement-able-item-id', cashFlowStatementItemId).attr('data-is-percentage-of', is_percentage_of).attr('data-percentage-or-fixed', percentage_or_fixed).attr('data-is-percentage', is_percentage).attr('data-is-repeating-fixed', is_repeating_fixed).attr('data-is-none-repeating-fixed', is_non_repeating_fixed).attr('data-is-trigger-change', 'true');

                                    $('tr.maintable-1-row-class' + cashFlowStatementItemId + '[data-sub-item-name="' + oldSubItemName + '"] td.editable-date:eq(0)').html(number_format(tdValue)).trigger('blur');
                ...
                function formatDatesForInterval(intervalName) {
                    ...
                    if ($('#cash-flow-statement-duration-type').val() != intervalName) {
