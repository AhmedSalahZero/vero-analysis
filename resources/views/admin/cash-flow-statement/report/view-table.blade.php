        ...
        <table class="table kt_table_with_no_pagination_no_collapse table-striped- table-bordered table-hover table-checkable position-relative table-with-two-subrows main-table-class dataTable no-footer">
            <thead>
                <tr class="header-tr " data-model-name="{{ $modelName }}">
                    ...
                    <th data-is-actual="{{ (int)isActualDate($defaultDateFormate) }}" data-date="{{ $defaultDateFormate }}" data-month-year="{{explode('-',$defaultDateFormate)[0].'-'.explode('-',$defaultDateFormate)[1]}}" class="view-table-th editable-date header-th" data-is-collection-relation="0" data-collection-item-id="0" data-db-column-name="name" data-relation-name="ServiceCategory" data-is-relation="1" class="header-th" data-is-json="0">
                        {{ $interval }}
            ...
            <tbody>
				...
				@php
					...
					$hasSubRows = $rowIndex < 2 ;
				@endphp
                <tr @if($isMonthlyRow) data-financial-statement-able-item-id="78" @endif class="@if($isAccumulatedRow) main-with-no-child even @endif   reset-table-width text-nowrap  cursor-pointer sub-text-bg text-capitalize is-close  " data-model-id="{{ $rowIndex }}">
                ...
                <tr class="edit-info-row add-sub maintable-1-row-class{{ $rowIndex }} is-sub-row d-none">
                    ...
                    @foreach($cashFlowStatement->getIntervalFormatted() as $defaultDateFormate=>$interval)
					@php
						$totalAtDate = $dateAndValues[$defaultDateFormate] ?? 0 ;
