...
<form class="kt-form kt-form--label-right" id="create-form" method="POST" action="{{ route('calculate.fixed.at.end.and.beginning',['company' => $company->id]) }}">
    {{ csrf_field() }}
                    ...
                    <div class="form-group validated">
                        ...
                        <div class="form-group-sub">
                            ...
                            <select name="nature_type" id="nature_type" class="form-control">
                                @foreach(['fixed_at_end'=>__('Fixed At End') ,'fixed_at_beginning'=>__('Fixed At Beginning')] as $natureType => $natureTitle)
                    ...
                    <div class="form-group validated">
                        ...
                        <div class="form-group-sub">
                            <select name="installment_interval" id="installment_interval" class="form-control installment_condition">
                                ...
                                <option @if($loan && $loan->installment_interval == 'quarterly')
                                    selected
...
<div>
@if(isset($fixedAtEndResult) && count($fixedAtEndResult))
<table class='table table-striped table-bordered table-hover table-checkable'>
	<thead>
		   <th class="text-center font-weight-bold">{{__("Payment No.")}}</th>
                            ...
                            <th class="text-center font-weight-bold">{{__("Principle Amount")}}</th>
	...
	{{-- {{ dd($fixedAtEndResult['beginning'],$loanStartDate,$loanDates) }} --}}
		...
		<tr>
			...
			<td class="text-center">{{ number_format($fixedAtEndResult['principleAmount'][$date] ?? 0) }}</td>
			<td class="text-center">{{ number_format($fixedAtEndResult['endBalance'][$date] ?? 0) }}</td>
		</tr>
		@php
			$index++;
		@endphp
		...
		<tr class="custom-color-for-last-tr">
			...
			<th class="text-center"> {{ number_format($fixedAtEndResult['totals']['totalSchedulePayment'] ?? 0) }} </th>
			<th class="text-center"> {{ number_format($fixedAtEndResult['totals']['totalInterestAmount'] ?? 0) }} </th>
			<th class="text-center"> {{ number_format($fixedAtEndResult['totals']['totalPrincipleAmount'] ?? 0 )}} </th>
					<th>-</th>
