            ...
            <div class="row">
                ...
                <div class="col-md-4 item-main-parent" style="display: none">
                    <div class="form-group validated">
                        ...
                        <div class="form-group-sub">
                            <input type="text" step="any" id="grace_periodid" name="grace_period" value="{{ @old('grace_period') }}" class="form-control number grace_period_calc installment_condition" placeholder="{{__('Grace Period')}} .." />
                    ...
                    <div class="form-group validated">
                        ...
                        <div class="form-group-sub">
                            <select name="step_up_interval" id="step_up_interval" class="form-control interval-calcs">
                                ...
                                <option value="quartly" {{ @old('step_up_interval') == 'quartly' ? 'selected' : '' }} data-order="1">{{__('Quarterly')}}</option>
                    ...
                    <div class="form-group validated">
                        ...
                        <div class="form-group-sub">
                            <select name="step_down_interval" id="step_down_interval" class="form-control interval-calcs">
                                <option value="" selected disabled>{{__('Select')}} ..</option>
                                <option value="quartly" {{ @old('step_down_interval') == 'quartly' ? 'selected' : '' }} data-order="1">{{__('Quarterly')}}</option>
                    ...
                    <div class="form-group validated">
                        ...
                        <div class="form-group-sub">
                            <select name="installment_interval" id="installment_interval" class="form-control">
                                ...
                                <option value="quartly" {{ @old('installment_interval') == 'quartly' ? 'selected' : '' }}>{{__('Quarterly')}}</option>
...
<script>
    $(document).on('click', '.submit', function(e) {
        ...
        e.preventDefault();
        calcIntersetPercentage();

    })


    function calcIntersetPercentage(calcedPrice = null) {
        ...
        let installmentAmount = $('#loan_amount').val() ? parseFloat($('#loan_amount').val()) : 0;

        let givenLoanAmount = $('#given_loan').val() ? parseFloat($('#given_loan').val()) : 0;
        ...
        while (!(givenLoanAmount == getLoanVal) && !(getLoanVal >= (0.99999 * givenLoanAmount) && (getLoanVal <= (1.00001 * givenLoanAmount))))
        {
            ...
            newPrice = calcedPrice ? calcedPrice : TrialInterest;

            if (operation == '+') {
                pricing = newPrice + 0.005;
            }
            if (operation == '-') {
                pricing = newPrice - 0.0001;
            }
            ...
            calcedPrice = pricing;
            intersetFactor = calcIntersetFactor(daysCount, pricing);
            loanFactories = calcLoanFactor(fixedType, loanAmount, intersetFactor, gracePeriod, installment_payment_interval, new Date(start_date_formatted.getTime()), period
                , addMonths(new Date(start_date_formatted.getTime()), (period ? period : 0))

            );

            installmentFactories = calcInstallmentFactor(new Date(installmentStartDate.getTime()), intersetFactor, stepRate, stepFactor, period, installment_payment_interval);

            installmentAmountArr = getInstallmentAmount(loanFactories, installmentFactories, stepRate, stepFactor, new Date(installmentStartDate.getTime()), addMonths(new Date(start_date_formatted.getTime()), (period ? period : 0))
                , period, installment_payment_interval, interval, loanAmount

            );
            getLoanVal = calcLoan(loanFactories, installmentFactories, stepRate, stepFactor, new Date(installmentStartDate.getTime()), addMonths(new Date(start_date_formatted.getTime()), (period ? period : 0))
                , period, installment_payment_interval, interval, loanAmount
            );
        }
    ...
    function getPercentageFormatted(number, decimals = 2) {
    ...
    function calcTrialInterest(gracePeriod, installment_payment_interval, period, stepRate, installmentAmount, givenLoanAmount, interval) {
        givenInstallmentAmountArr = [];
        ...
        totalGivenInstallment = givenInstallmentAmountArr.reduce(function(previousValue, currentValue) {
        ...
        $('#dynamic-datatable').DataTable({
            ...
            , buttons: ['copy', 'csv'
                , {
                    "extend": "excel"
                    , title: ''
                    , filename: 'Calculate Interest Percentage'
