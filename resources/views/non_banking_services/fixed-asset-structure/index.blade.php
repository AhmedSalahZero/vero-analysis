    ...
    <div class="kt-portlet__head">
        <div class="kt-portlet__head-toolbar justify-content-between flex-grow-1">
            <ul class="nav nav-tabs nav-tabs-space-lg nav-tabs-line nav-tabs-bold nav-tabs-line-3x nav-tabs-line-brand" role="tablist">
                <li class="nav-item">
                    <a class="nav-link {{ !Request('active') || Request('active') == FixedAssetName::FIXED_ASSET ?'active':'' }}" data-toggle="tab" href="#{{FixedAssetName::FIXED_ASSET  }}" role="tab">
                        <i class="fa fa-money-check-alt"></i> {{ __('Fixed Asset Names') }}
                    </a>
                </li>
            ...
            {{-- @if(auth()->user()->can('create study info')) --}}
			 ...
			 <div class="flex-tabs">
                ...
                <a href="{{ route('create.fixed.asset.names',['company'=>$company->id]) }}" class="btn btn-2-bg bg-white-hover new-study-item rounded btn-icon-sm align-self-center">
                                    ...
                                    <td>
									  ...
									  <td class="kt-datatable__cell--left kt-datatable__cell " data-field="Actions" data-autohide-disabled="false">
                                        <span style="overflow: visible; position: relative; width: 110px;">
                                            ...
                                            <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon edit-btn-class" title="{{ __('Edit') }}" href="{{ route('edit.fixed.asset.names',['company'=>$company->id,'fixedAssetName'=>$model->id]) }}"><i class="fa fa-pen-alt exclude-icon default-icon-color" ></i></a>
                                            ...
                                            <a data-toggle="modal" data-target="#delete-fixed-asset-{{ $model->id }}" type="button" class="btn delete-btn-class btn-secondary btn-outline-hover-danger btn-icon" title="Delete" href="#"><i class="fa fa-trash-alt exclude-icon default-icon-color"></i></a>
                                            <div class="modal fade" id="delete-fixed-asset-{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content">
                                                        <form action="{{ route('fixed.asset.names.destroy',['company'=>$company->id,'fixedAssetName'=>$model->id ]) }}" method="post">
                                                            @csrf
                                                            @method('delete')
