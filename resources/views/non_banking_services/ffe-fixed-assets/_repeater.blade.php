<div data-card-id="{{ $cardId }}" class="kt-portlet parent-card ">
    <div class="kt-portlet__body">
        ...
        @include('non_banking_services.fixed-assets._input-hidden') --}}
        {{-- start of fixed monthly repeating amount --}}


        <input type="hidden" name="tableIds[]" value="{{ $tableId }}">

        <x-tables.repeater-table :initEmpty="false" :removeActionBtn="false" :first-element-deletable="false" :font-size-class="'font-14px'" :append-save-or-back-btn="true" :repeater-with-select2="true" :parentClass="'js-toggle-visibility-----'" :tableName="$tableId " :repeaterId="$repeaterId" :relationName="'food'" :isRepeater="$isRepeater=!(isset($removeRepeater) && $removeRepeater)">
            <x-slot name="ths">
            ...
            <x-slot name="trs">
                ...
                <tr data-repeater-item data-repeat-formatting-decimals="2" data-repeater-style>
                    ...
                    <td>
                        <div class="">
                            <input value="{{ isset($subModel) ? $subModel->getFfeItemCost() : 0 }}" @if($isRepeater) name="ffe_item_cost" @else name="{{ $tableId }}[0][ffe_item_cost]" @endif class="form-control expandable-amount-input text-left " type="text">
                        </div>
                    </td>
                    ...
                    <td>
                        ...
                        <div class="d-flex align-items-center">
                            <input value="{{ isset($subModel) ? $subModel->getVatRate():0 }}" @if($isRepeater) name="vat_rate" @else name="{{ $tableId }}[0][vat_rate]" @endif class="form-control expandable-percentage-input text-left " type="text">
                    ...
                    <td>
                        ...
                        <div class="d-flex align-items-center">
                            <input value="{{ isset($subModel) ? $subModel->getWithholdTaxRate():0 }}" @if($isRepeater) name="withhold_tax_rate" @else name="{{ $tableId }}[0][withhold_tax_rate]" @endif class="form-control expandable-percentage-input text-left " type="text">
                            <span style="margin-left:3px	">%</span>
                        </div>
                    </td>
                    <td>
                        ...
                        <div class="d-flex align-items-center">
                            <input value="{{ isset($subModel) ? $subModel->getCostAnnualIncreaseRate():0 }}" @if($isRepeater) name="cost_annual_increase_rate" @else name="{{ $tableId }}[0][cost_annual_increase_rate]" @endif class="form-control expandable-percentage-input text-left " type="text">
                            <span style="margin-left:3px	">%</span>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <x-form.select :selectedValue="isset($subModel) ? $subModel->getPaymentTerm() : 'cash'" :options="getDepreciationDurations()" :add-new="false" class="select2-select repeater-select payment_terms " :all="false" name="@if($isRepeater) payment_terms @else {{ $tableId }}[0][payment_terms] @endif"></x-form.select>
                    </td>
                    <td>
                    ...
                    <td>
                        <x-form.select :selectedValue="isset($subModel) ? $subModel->getPaymentTerm() : 'cash'" :options="getReplacementInterval()" :add-new="false" class="select2-select repeater-select payment_terms " :all="false" name="@if($isRepeater) payment_terms @else {{ $tableId }}[0][payment_terms] @endif"></x-form.select>
                    </td>
                    ...
                    <td data-column-index="{{ $dateAsIndex }}">
                        <div class="d-flex align-items-center justify-content-center">
                            ...
                            @endphp
                            <x-repeat-right-dot-inputs :removeCurrency="true" :removeThreeDots="true" :removeThreeDotsClass="true" :number-format-decimals="0" :mark="' '" :currentVal="isset($subModel) ? $subModel->getFfeCountsAtDateIndex($dateAsIndex) : 0 " data-group-index="{{ $currentYearRepeaterIndex }}" :classes="'repeater-with-collapse-input only-greater-than-or-equal-zero-allowed '" :is-percentage="true" :name="$name" :columnIndex="$columnIndex"></x-repeat-right-dot-inputs>
                        </div>
                    </td>
                    @php
                    $currentMonthNumber = explode('-',$dateAsString)[1];
 ...
 <div class="form-group d-inline-block">
                     <div class="kt-radio-inline">
                         ...
                         <label class="kt-radio kt-radio--success text-black font-size-18px font-weight-bold">
                             ...
                             <input type="radio" value="1" name="is_fully_funded_though_equity" @if(!isset($subModel) || (isset($subModel) && $subModel->isFullyFundedThroughEquity())) checked @endisset
                             > {{ __('Fully Funded Through Equity') }}
                             <span></span>
                         </label>

                         <label class="kt-radio kt-radio--danger text-black font-size-18px font-weight-bold">
                             <input type="radio" value="0" name="is_fully_funded_though_equity" @if(isset($subModel) && !$subModel->isFullyFundedThroughEquity()) checked @endisset
