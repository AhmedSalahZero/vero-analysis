...
<td>
                                            ...
                                            if($isByBranch){
                                            $currentVal = $productMix->early_payment_installment_counts;
                                            }
                                            @endphp
                                            <x-repeat-right-dot-inputs :numberFormatDecimals="0" :readonly="$isByBranch" :removeThreeDots="true" :removeCurrency="true" :currentVal="$currentVal" :classes="'only-greater-than-or-equal-zero-allowed'" :is-percentage="true" :name="'microfinanceProductSalesProjects['.$product->id.'][early_payment_installment_counts]'" :columnIndex="-1"></x-repeat-right-dot-inputs>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            if($isByBranch){
                                            $currentVal = $productMix->avg_amount;
                                            }
                                            @endphp
                        ...
                        <div class="table-responsive">
                            <table class="table table-white repeater-class repeater ">
                                ...
                                <tbody>
                                    ...
                                    $subModel = $model->microfinanceProductSalesProjects->where('type',$type)->where('branch_id',$branchId)->where('microfinance_product_id',$product->id)->first();
                                    @endphp
                                    <tr data-repeat-formatting-decimals="3" data-repeater-style>
                                        <td class="td-classes">
                                            <div>
                                                <input value="{{ $product->getName() }}" disabled="" class="form-control text-left min-w-300" type="text">
                                            </div>
                                        ...
                                        @for($i = 1 ; $i<= 12 ; $i++ ) @php $i=sprintf("%02d", $i); @endphp <td data-product-id="{{ $product->id }}">
                                            ...
                                            <x-repeat-with-calc :showIcon="false" :numberFormatDecimals="3" :formattedInputClasses="'calcField'" :mark="'%'" :removeThreeDots="false" :removeCurrency="true" :currentVal="number_format($currentVal,3)" :classes="'seasonality-class'" :is-percentage="true" :name="'microfinanceProductSalesProjects['.$product->id.'][seasonality]['.$i.']'" :columnIndex="$columnIndex"></x-repeat-with-calc>
                    ...
                    <x-tables.repeater-table :hideByDefault="false" :removeActionBtn="true" :removeRepeater="true" :initialJs="false" :repeater-with-select2="true" :canAddNewItem="false" :parentClass="'js-remove-hidden overflow-scroll'" :hide-add-btn="true" :repeater-with-select2="true" :parentClass="''" :tableName="$tableId" :repeaterId="$repeaterId" :relationName="'food'" :isRepeater="$isRepeater=false">
                        ...
                        <x-slot name="trs">
                            ...
                            <tr data-repeater-style>
                                ...
                                $columnIndex = 0 ;
                                @endphp
                                @for($i = 0 ; $i<= $months ; $i++) @php $currentVal=isset($subModel) ? $subModel->getNewLoanCasesAtYearOrMonthIndex($i,$isSenior) : 0 ;
                                    $currentVal = $isByBranch ? $study->getProductMixSeniorLoanOfficersAt($i,$isSenior) : $currentVal;
            ...
            <div class="kt-portlet">
                ...
                <div class="kt-portlet__body">
                    ...
                    <h3 class="font-weight-bold form-label kt-subheader__title small-caps mr-5" style=""> {{ __('Branches Manpower Hiring Projection') }} </h3>
                    <div class="row">
                        <hr style="flex:1;background-color:lightgray">
                    </div>
            ...
            <div class="kt-portlet">
                ...
                <div class="kt-portlet__body">
                    <div class="row">
                        <div class="col-md-10">
                            <h3 class="font-weight-bold form-label kt-subheader__title small-caps mr-5" style=""> {{ __('Branches Expense (Please Insert Avg. Expense Per Branch)') }} </h3>
                    ...
                    <div class="form-group row justify-content-center">
                        ...
                        <x-tables.repeater-table :triggerInputChangeWhenAddNew="true" :hideByDefault="false" :removeRepeater="false" :repeater-with-select2="true" :parentClass="'expenses-table js-toggle-visibility'" :tableName="$tableId" :repeaterId="$repeaterId" :relationName="'food'" :isRepeater="$isRepeater=!(isset($removeRepeater) && $removeRepeater)">
                            ...
                            <x-slot name="trs">
                                @php
                                $rows = isset($model) ? $model->generateRelationDynamically($tableId,$expenseType)->where('branch_id',$branchId)->get() : [-1] ;
                                @endphp
