            ...
            <div class="tab-pane {{  !Request('active') || Request('active') == $currentType ?'active':'' }}" id="{{ $currentType }}" role="tabpanel">
                <div class="kt-portlet kt-portlet--mobile">
                    <x-table-title.with-two-dates :type="$currentType" :title="__('Subsidary Companies')" :startDate="$filterDates[$currentType]['startDate']??''" :endDate="$filterDates[$currentType]['endDate']??''">
                        <x-export-subsidiary-companies :indexRouteName="$indexRouteName" :search-fields="$searchFields[$currentType]" :money-received-type="$currentType" :has-search="1" :has-batch-collection="0" href="{{route('subsidiary.companies.create',['company'=>$company->id])}}" />
                                   ...
                                   <td class="text-nowrap text-left">{{ $model->getName() }}</td>
                                    ...
                                    <td class="kt-datatable__cell--left kt-datatable__cell " data-field="Actions" data-autohide-disabled="false">
                                        <span style="overflow: visible; position: relative; width: 110px;">
											@if(hasAuthFor('update subsidiary companies'))
                                            <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon" title="Edit" href="{{ route('subsidiary.companies.edit',['company'=>$company->id,'subsidiaryCompany'=>$model->id]) }}"><i class="fa fa-pen-alt"></i></a>
											@endif 
											{{-- @if(hasAuthFor('delete subsidiary companies'))
                                            <a data-toggle="modal" data-target="#delete-subsidiaryCompany-{{ $model->id }}" type="button" class="btn btn-secondary btn-outline-hover-danger btn-icon" title="Delete" href="#"><i class="fa fa-trash-alt"></i></a>
                                            <div class="modal fade" id="delete-subsidiaryCompany-{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content">
                                                        <form action="{{ route('subsidiary.companies.destroy',['company'=>$company->id,'subsidiaryCompany'=>$model->id ]) }}" method="post">
                                                            @csrf
                                                            @method('delete')
