    ...
    <div class="kt-portlet__head">
        <div class="kt-portlet__head-toolbar justify-content-between flex-grow-1">
            <ul class="nav nav-tabs nav-tabs-space-lg nav-tabs-line nav-tabs-bold nav-tabs-line-3x nav-tabs-line-brand" role="tablist">
                <li class="nav-item">
                    <a class="nav-link {{ !Request('active') || Request('active') == Partner::CUSTOMERS ?'active':'' }}" data-toggle="tab" href="#{{Partner::CUSTOMERS  }}" role="tab">
                        <i class="fa fa-money-check-alt"></i> {{ __('Customers Table') }}
                    </a>
                </li>
            ...
            <div class="flex-tabs">
                ...
                <a href="{{ route('customers.create',['company'=>$company->id,Partner::CUSTOMERS]) }}" class="btn  active-style btn-icon-sm align-self-center">
        ...
        <div class="tab-content  kt-margin-t-20">
            ...
            <div class="tab-pane {{  !Request('active') || Request('active') == $currentType ?'active':'' }}" id="{{ $currentType }}" role="tabpanel">
                <div class="kt-portlet kt-portlet--mobile">
                    <x-table-title.with-two-dates :type="$currentType" :title="__('Customers')" :startDate="$filterDates[$currentType]['startDate']??''" :endDate="$filterDates[$currentType]['endDate']??''">
                        <x-export-customers :indexRouteName="$indexRouteName" :search-fields="$searchFields[$currentType]" :money-received-type="$currentType" :has-search="1" :has-batch-collection="0" href="{{route('customers.create',['company'=>$company->id])}}" />
                                            ...
                                            <div class="modal fade" id="delete-customer-{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content">
                                                        <form action="{{ route('customers.destroy',['company'=>$company->id,'customer'=>$model->id ]) }}" method="post">
                                                            @csrf
                                                            @method('delete')
