    ...
    <div class="kt-portlet__head">
        <div class="kt-portlet__head-toolbar justify-content-between flex-grow-1">
            <ul class="nav nav-tabs nav-tabs-space-lg nav-tabs-line nav-tabs-bold nav-tabs-line-3x nav-tabs-line-brand" role="tablist">
                <li class="nav-item">
                    <a class="nav-link {{ !Request('active') || Request('active') == CashVeroSalesChannel::SALES_CHANNELS ?'active':'' }}" data-toggle="tab" href="#{{CashVeroSalesChannel::SALES_CHANNELS  }}" role="tab">
            ...
            <div class="flex-tabs">
                ...
                <a href="{{ route($createRouteName,['company'=>$company->id,CashVeroSalesChannel::SALES_CHANNELS]) }}" class="btn  active-style btn-icon-sm align-self-center">
            ...
            <div class="tab-pane {{  !Request('active') || Request('active') == $currentType ?'active':'' }}" id="{{ $currentType }}" role="tabpanel">
                <div class="kt-portlet kt-portlet--mobile">
                    <x-table-title.with-two-dates :type="$currentType" :title="$title" :startDate="$filterDates[$currentType]['startDate']??''" :endDate="$filterDates[$currentType]['endDate']??''">
                        <x-export-sales-channels :indexRouteName="$indexRouteName" :search-fields="$searchFields[$currentType]" :money-received-type="$currentType" :has-search="1" :has-batch-collection="0" href="{{$createRoute}}" />
                                ...
                                <tr>
                                    ...
                                    <td class="kt-datatable__cell--left kt-datatable__cell " data-field="Actions" data-autohide-disabled="false">
                                        <span style="overflow: visible; position: relative; width: 110px;">
                                            ...
                                            <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon" title="Edit" href="{{ route($editModelName,['company'=>$company->id,'salesChannel'=>$model->id]) }}"><i class="fa fa-pen-alt"></i></a>
											@endif 
                                            ...
                                            <div class="modal fade" id="delete-customer-{{ $model->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content">
                                                        <form action="{{ route($deleteRouteName,['company'=>$company->id,'salesChannel'=>$model->id ]) }}" method="post">
                                                            @csrf
                                                            @method('delete')
