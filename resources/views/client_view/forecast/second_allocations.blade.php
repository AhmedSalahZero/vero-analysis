...
<form action="{{ route('second.allocations', $company) }}" method="POST">
                        ...
                        <div class="kt-portlet__body">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <label>{{ __('Select Second Allocation Base') }} <span class="required">*</span></label>
                                    <div class="kt-input-icon">
                                        <div class="input-group date validated">
                                            <select name="allocation_base" class="form-control" id="allocation_base">
                                                ...
                                                <option value="branch" {{ @$allocations_setting['allocation_base'] !== 'branch' ?: 'selected' }}>
                                                    {{ __('Branches') }}</option>
                                                @endif
                                                ...
                                                <option value="business_sector" {{ @$allocations_setting['allocation_base'] !== 'business_sector' ?: 'selected' }}>
                                                ...
                                                <option value="sales_channel" {{ @$allocations_setting['allocation_base'] !== 'sales_channel' ?: 'selected' }}>
                                                ...
                                                <option value="zone" {{ @$allocations_setting['allocation_base'] !== 'zone' ?: 'selected' }}>
                                                    {{ __('Zones') }}</option>
                                                @endif
                                            </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group  form-group-marginless">
                                        <label>{{ __('Select Sales Breakdown %') }} <span class="required">*</span></label>
                                        <div class="kt-input-icon">
                                            <div class="input-group date validated">
                                                <select name="breakdown" class="form-control" id="breakdown">
                                                    <option value="" disabled selected>{{ __('Select') }}</option>
                                                    <option value="previous_year" {{ @$allocations_setting['breakdown'] !== 'previous_year' ?: 'selected' }}>
                                                        {{ __('Pervious Year Breakdown / New Breakdown') }}</option>
                                                    <option value="last_3_years" {{ @$allocations_setting['breakdown'] !== 'last_3_years' ?: 'selected' }}>
                                ...
                                <div class="col-md-6" style="display: {{ @$allocations_setting['new_start'] == 'previous_year' || @$allocations_setting['new_start'] == 'previous_3_years'? 'block': 'none' }}" id="new_start_field">
                                    <div class="form-group  form-group-marginless">
                                                    ...
                                                    <label class="kt-option">
                                                        <span class="kt-option__control">
                                                            <span class="kt-radio kt-radio--bold kt-radio--brand kt-radio--check-bold" checked>
                                                                <input type="radio" name="new_start" value="annual_target" {{ @$section_row['new_start'] == 'annual_target' ? 'checked' : '' }}>
                                                                <span></span>
                                                            </span>
                                                        </span>
                                                    ...
                                                    <label class="kt-option">
                                                        <span class="kt-option__control">
                                                            <span class="kt-radio kt-radio--bold kt-radio--brand">
                                                                <input type="radio" name="new_start" value="product_target" {{ @$section_row['new_start'] == 'product_target' ? 'checked' : '' }}>
                                                                <span></span>
                                                            </span>
                                                        </span>
                                                        <span class="kt-option__label">
                            ...
                            <div class="row">
                                    ...
                                    <label class="kt-option bg-secondary">
                                        <span class="kt-option__control">
                                            <span class="kt-checkbox kt-checkbox--bold kt-checkbox--brand kt-checkbox--check-bold" checked>
                                                <input class="rows" name="add_new_items" type="checkbox" value="1" readonly {{ @$allocations_setting['add_new_items'] == 0 ?: 'checked' }} id="add_new_items">
                                                <span></span>
                                            </span>
                                        </span>
                                ...
                                <div class="col-md-6" style="display:{{ @$allocations_setting['add_new_items'] == 1 ? 'block' : 'none' }}" id="number_of_items">
                                    <label>{{ __('How Many ? ') }} <span class="required">*</span></label>
                                    <div class="kt-input-icon">
                                        <div class="input-group validated">
                                            <input type="number" step="any" class="form-control" name="number_of_items" value="{{ @$allocations_setting['number_of_items'] }}" id="number_of_items">
    ...
    <div class="kt-portlet__body">
        ...
        <x-table :tableTitle="__($item.' Sales Targets Values Table')" :tableClass="'kt_table_with_no_pagination'">
            @slot('table_header')
        ...
        <x-table :tableTitle="__($item.' Sales Target Values Table')" :tableClass="'kt_table_with_no_pagination'">
            ...
            <tr>
                ...
                <td class="text-center">
                    <input type="number" name="modify_sales_target[{{ $product_data['item'] }}][percentage]" placeholder="{{ __('%') }}" class="modify_sales_target_percentage form-control" value="{{ @$modified_targets['products_modified_targets'][$product_data['item']]['percentage'] }}">
                </td>
                @else
                @foreach ($sales_targets as $quarter_name => $value)
                ...
                <td class="text-center">
                    <input type="number" name="modify_sales_target[{{ $product_data['item'] }}][{{ $quarter_name }}][value]" placeholder="{{ __('Value') }}" class="modify_sales_target_{{ $quarter_name }} form-control" data-quarter_annual_target="{{ $value }}" value="{{ @$modified_targets['products_modified_targets'][$product_data['item']][$quarter_name]['value'] }}">
                </td>
                <td class="text-center">
                    <input type="number" name="modify_sales_target[{{ $product_data['item'] }}][{{ $quarter_name }}][percentage]" data-quarter_annual_target="{{ $value }}" placeholder="{{ __('%') }}" class="modify_sales_target_percentage_{{ $quarter_name }} form-control" value="{{ @$modified_targets['products_modified_targets'][$product_data['item']][$quarter_name]['percentage'] }}">
                </td>
                @endforeach
                @endif
            </tr>
