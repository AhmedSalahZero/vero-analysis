...
@if(config('app.showTrendCharts'))
           ...
           @endif 
            ...
            <div class="tab-pane @if(!config('app.showTrendCharts')) active @endif" id="kt_apps_contacts_view_tab_2 " role="tabpanel">
                <x-table :tableTitle="__('Zone Sales Trend Analysis Report')" :tableClass="'kt_table_with_no_pagination_no_search'">
                    @slot('table_header')
                    ...
                    <tr class="group-color  text-lg-left  ">
                        <td colspan="{{ count($total_zones) + 1 }}"><b class="white-text">{{ __($zone_name) }}</b>
                        </td>
                        @foreach ($total_zones as $date => $total)
                        <td class="hidden"> </td>
                        @endforeach
                ...
                <x-table :tableTitle="__('Zone Sales Percentage (%) Against Total Sales')" :tableClass="'kt_table_with_no_pagination_no_search'">
                    @slot('table_header')
                    <tr class="table-active">
                        <th>{{ __('Zone') }}</th>
                ...
                @include('seasonality_table' , ['total_branches'=>$total_zones , 'totalArrys'=>[]])
