    ...
    <div class="kt-portlet__body">
        <div class="tab-content  kt-margin-t-20">
            ...
            <div class="tab-pane" id="kt_apps_contacts_view_tab_2" role="tabpanel">
                <x-table :tableTitle="__('Country Sales Trend Analysis Report')" :tableClass="'kt_table_with_no_pagination_no_search'">
                    @slot('table_header')
                    <tr class="table-active">
                        <th>{{ __('Country') }}</th>
                        @foreach ($total_countries as $date => $total)
                    ...
                    <tr class="group-color  text-lg-left  ">
                        <td colspan="{{ count($total_countries) + 1 }}"><b class="white-text">{{ __($zone_name) }}</b>
                        </td>
                        @foreach ($total_countries as $date => $total)
                        <td class="hidden"> </td>
                        @endforeach
                    ...
                    <tr>
                        ...
                        <?php $chart_data = []; ?>
                        @foreach ($total_countries_growth_rates as $date => $total)
                        <?php
                                    $chart_data[] = [
                                        'date' => date('d-M-Y', strtotime($date)),
                ...
                <x-table :tableTitle="__('Country Sales Percentage (%) Against Total Sales')" :tableClass="'kt_table_with_no_pagination_no_search'">
                    @slot('table_header')
                    <tr class="table-active">
                        <th>{{ __('Country') }}</th>
                    ...
                    @foreach ($final_report_data as $zone_name => $zoone_data)
                    <tr class="group-color  text-lg-left  ">
                        <td colspan="{{ count($total_countries) + 1 }}"><b class="white-text">{{ __($zone_name) }}</b></td>
                        @foreach ($total_countries as $date => $total)
                        <td class="hidden"> </td>
                        @endforeach
                    </tr>
...
<script>
    am4core.ready(function() {
        ...
        if ("{{ $name_of_zone }}" == 'Country_Sales_Percentages') {
