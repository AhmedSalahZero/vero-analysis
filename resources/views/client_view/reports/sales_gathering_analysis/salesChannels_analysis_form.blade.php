    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form class="kt-form kt-form--label-right" method="POST"
                action=@if($name_of_selector_label == 'Sales Discount') {{ route('salesChannels.salesDiscount.analysis.result', $company) }}
                        @elseif ($type == 'averagePrices' || ($type == 'averagePricesProductItems') ) {{ route('averagePrices.result', $company) }}
                        @else {{route('salesChannels.analysis.result', $company) }}
                        @endif
  ...
  wto = setTimeout(()=> {
   ...
   if(tryParseJSONObject($('#salesChannels').val()[0])){
        ...
        function getSalesChannales(salesChannels,type_of_data) {
            $.ajax({
            type:'GET',
            data: {'main_data' : salesChannels , 'main_field' : 'sales_channel','field' : type_of_data} ,
