...
<form action="{{ route('multipleRowsDelete', [$company, 'SalesGathering']) }}" method="POST">
    @csrf
    <x-table :tableTitle="__('Sales Table')" :tableClass="'kt_table_with_no_pagination'" href="#" :importHref="$user->can('upload sales gathering data') ? route('salesGatheringImport',$company) : '#'" :exportHref="$user->can('export sales gathering data') ? route('salesGathering.export',$company):'#' " :exportTableHref="$user->can('upload sales gathering data')?route('table.fields.selection.view',[$company,'SalesGathering','sales_gathering']) : '#'" :truncateHref="$user->can('delete sales gathering data')?route('truncate',[$company,'SalesGathering']):'#' ">
        @slot('table_header')
		
        <x-modal.delete :method="'get'" :title="__('Do You Want To Delete All Your Sales Data ?')" :deleteRoute="$user->can('delete sales gathering data')?route('truncate',[$company,'SalesGathering']):'#'"></x-modal.delete>
		@if($user->can('delete sales gathering data'))
        <div class="col-md-12">
            <div class="row">
                <div class="col-lg-12 ">
