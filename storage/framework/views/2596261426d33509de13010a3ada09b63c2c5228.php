    <script>
                $(document).on('change', '[change-financial-instutition-js]', function() {
                    ...
                    let url = "<?php echo e(route('update.balance.and.net.balance.based.on.account.id.ajax',['company'=>$company->id , 'accountType'=>'replace_account_type' , 'accountId'=>'replace_account_id','financialInstitutionId'=>'replace_financial_institution_id' ])); ?>";
				...
				$(document).on('change', '[js-when-change-trigger-change-account-type]', function () {
					...
					$(document).on('change','.recalculate-cd-or-td-free-to-use',function(){
						...
						const amount = cdOrTdAmount-currentLgOutstandBalance ;
