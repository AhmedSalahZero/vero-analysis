...
<div class="table-custom-container position-relative  ">
    ...
    <script>
		let datesWithIsActual = <?php echo json_encode($actualDates, 15, 512) ?>;
...
<?php $component->withAttributes(['redirect-route' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(route('admin.view.financial.statement', getCurrentCompanyId())),'save-and-return' => true,'form-id' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('store-report-form-id'),'wrap-with-form' => true,'form-action' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(route('admin.store.income.statement.report',['company'=>getCurrentCompanyId()])),'class' => 'position-relative table-with-two-subrows main-table-class','id' => ''.e($tableId).'']); ?>
         ...
         <?php $__env->slot('headerTr'); ?> 
            ...
            <tr class="header-tr " data-model-name="<?php echo e($modelName); ?>">
                ...
                <input type="hidden" name="dates" data-date-formatted="<?php echo e(json_encode((collect($incomeStatement->getIntervalFormatted())->map(function($date){return formatDateForView($date);})->toArray()))); ?>" data-formatted="<?php echo e(json_encode(($incomeStatement->getIntervalFormatted()))); ?>" value="<?php echo e(json_encode(array_keys($incomeStatement->getIntervalFormatted()))); ?>" id="dates">
                <?php $__currentLoopData = $incomeStatement->getIntervalFormatted(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dateAsIndex=>$dateAsString): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
				
                <th data-is-actual="<?php echo e((int)isActualDate($dateAsString)); ?>" data-date="<?php echo e($dateAsIndex); ?>"  class="view-table-th header-th text-wrap" data-is-collection-relation="0" data-collection-item-id="0" data-db-column-name="name" data-relation-name="ServiceCategory" data-is-relation="1" class="header-th" data-is-json="0">
                    <?php echo e(formatDateForView($dateAsString)); ?>

                    <?php if(isActualDate($dateAsString) && $reportType != 'forecast'): ?>
                    <br>(<?php echo e(__('Actual')); ?>)
                    <?php elseif($reportType != 'forecast'): ?>
                    <br>(<?php echo e(__('Forecast')); ?>)
                    <?php endif; ?>
                    <?php if((int)isActualDate($dateAsString)): ?>
                    <div class="is-actual-dates" data-date="<?php echo e($dateAsIndex); ?>" style="visibility:hidden"></div>
                    <?php endif; ?>
                </th>
                                ...
                                function() {
                                    ...
                                    columns.push({
                                        render: function(d, b, row) {
                                            ...
                                            if (!row.isSubItem && row.has_sub_items) {
                                                elements = `<a data-is-subitem="0"    class=" ${row.id == domElements.corporateTaxesId ? 'd-none' :'d-block' } add-btn mb-2" href="#" data-toggle="modal" data-target="#add-sub-modal${row.id}"><?php echo e(__('Add')); ?></a> `;
                                                return elements;
                                            ...
                                            , buttons: [{
                                                ...
                                                , {
                                                    ...
                                                    , "action": function() {
                                                        ...
                                                        let exportFormAction = "<?php echo e(route('admin.export.income.statement.report',[$company->id,$incomeStatement->id,$reportType])); ?>";
...

                                                {
                                                    ...
                                                    , "action": function() {
                                                        ...
                                                        let exportFormAction = "<?php echo e(route('admin.export.income.statement.report.pdf',[$company->id,$incomeStatement->id,$reportType])); ?>";
                                                        form.attr('action', exportFormAction);
                                                        form.submit();
                                                        form.attr('action', oldFormAction);
                                                        ...
                                                        $(row).addClass('edit-info-row').addClass('add-sub maintable-1-row-class' + (data.id)).attr('data-model-id', data.id).attr('data-model-name', '<?php echo e($modelName); ?>')
                                                            .append(`
																			...
																			<div class="modal-dialog" role="document">
																				...
																				<div class="modal-body">
																					<form  data-financial-statement-able-item-id="${data.id}" id="add-sub-item-form${data.id}" class="submit-sub-item" action="<?php echo e(route('admin.store.income.statement.report',['company'=>getCurrentCompanyId()])); ?>" method="post">
																					...
																					<button type="button" class="btn btn-secondary" data-dismiss="modal"><?php echo e(__('Close')); ?></button>
																					<button type="submit" class="btn btn-primary save-sub-item" data-redirect-to='' data-id="${data.id}"><?php echo e(__('Save')); ?></button>
																				</div>
																				</form>
																				</div>
								...
								<div class="basis-100 for-only-one-checked ${isCustom ? 'd-none' : ''}" data-item="system_default">
										<div class="system-default-select">
											<select name="sub_items[0][collection_policy][type][system_default][value]" class="select form-control">
												<option ${isSystemDefault && pivot.collection_policy_value =='monthly' ? 'selected' : ''} value="monthly"><?php echo e(__('Cash')); ?></option>
												<option ${isSystemDefault && pivot.collection_policy_value =='quarterly' ? 'selected' : ''} value="quarterly"><?php echo e(__('Quarterly')); ?></option>
												<option ${isSystemDefault && pivot.collection_policy_value =='semi-annually' ? 'selected' : ''} value="semi-annually"><?php echo e(__('Semi-annually')); ?></option>
												<option ${isSystemDefault && pivot.collection_policy_value =='annually' ? 'selected' : ''} value="annually"><?php echo e(__('Annually')); ?></option>
											</select>
										</div>
								</div>
...
<?php /**PATH /media/salah/Software/projects/veroo/resources/views/admin/income-statement/report/view-table.blade.php ENDPATH**/ ?>