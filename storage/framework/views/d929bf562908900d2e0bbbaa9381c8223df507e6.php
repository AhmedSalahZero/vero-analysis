...
.btn-active i {
    ...
    .is-sub-row td.sub-numeric-bg,
    .is-sub-row td.sub-text-bg,
    tr[data-financial-statement-able-item-id="<?php echo e(\App\Models\CashFlowStatementItem::NET_CASH_PROFIT_ID); ?>"] td.sub-text-bg,
    tr[data-financial-statement-able-item-id="<?php echo e(\App\Models\CashFlowStatementItem::NET_CASH_PROFIT_ID); ?>"] td.sub-numeric-bg {
        background-color: #046187 !important;
        color: white !important;
...
<?php if(in_array($reportType,['modified','adjusted'])): ?>
...
<input type="hidden" id="monthly_data_monthly_net_cash" data-total="<?php echo e(json_encode($formattedDataForMonthlyNetCashChart ?? [])); ?>">
        ...
        <table class="table kt_table_with_no_pagination_no_collapse table-striped- table-bordered table-hover table-checkable position-relative table-with-two-subrows main-table-class dataTable no-footer">
            <thead>
                <tr class="header-tr " data-model-name="<?php echo e($modelName); ?>">
                    ...
                    <input type="hidden" name="dates" value="<?php echo e(json_encode(array_keys($cashFlowStatement->getIntervalFormatted()))); ?>" id="dates">
                     <?php $__currentLoopData = $cashFlowStatement->getIntervalFormatted(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dateAsIndex=>$dateAsString): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <th  class="view-table-th editable-date header-th" data-is-collection-relation="0" data-collection-item-id="0" data-db-column-name="name" data-relation-name="ServiceCategory" data-is-relation="1" class="header-th" data-is-json="0">
                         <?php echo e(formatDateForView($dateAsString)); ?>
            ...
            <tbody>
                ...
                <?php $__currentLoopData = $cashes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cashName => $elements): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php

                $isAccumulatedRow = $rowIndex ==3;
                $isMonthlyRow = $rowIndex ==2;
                $hasSubRows = $rowIndex < 2 ; ?> <tr <?php if($isMonthlyRow): ?> data-financial-statement-able-item-id="78" <?php endif; ?> class="<?php if($isAccumulatedRow): ?> main-with-no-child even <?php endif; ?>   reset-table-width text-nowrap  cursor-pointer sub-text-bg text-capitalize is-close  " data-model-id="<?php echo e($rowIndex); ?>">
                    <td class="red reset-table-width text-nowrap trigger-child-row-1 cursor-pointer sub-text-bg text-capitalize  is-close"> <?php if($hasSubRows): ?> + <?php endif; ?></td>
                    <td class="sub-text-bg  max-w-classes editable-text is-name-cell "><?php echo e($cashName); ?></td>
                    <?php $__currentLoopData = $cashFlowStatement->getIntervalFormatted(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dateIndex=>$dateAsString): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <td class="  sub-numeric-bg text-center editable-date" <?php if($isAccumulatedRow): ?> style="color:white !important" <?php endif; ?>><?php echo e(number_format($elements['total'][$dateAsString] ??0,0)); ?></td>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <td <?php if($isMonthlyRow): ?> class="sub-numeric-bg text-center" <?php endif; ?> <?php if($isAccumulatedRow): ?> style="color:white !important" <?php endif; ?>><?php echo e($isAccumulatedRow ? '-' : number_format(array_sum($elements['total'] ?? []),0)); ?></td>
                    </tr>
                    <?php
                    ...
                    <?php $__currentLoopData = $elements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $subName => $dateAndValues): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                    $currentTotalAtDate = 0;
                    ?>
                    <?php if($subName !='total'): ?>
                    <tr class="edit-info-row add-sub maintable-1-row-class<?php echo e($rowIndex); ?> is-sub-row d-none">
                        ...
                        <td class="sub-text-bg max-w-classes editable editable-text is-name-cell "><?php echo e($subName); ?></td>
                        <?php $__currentLoopData = $cashFlowStatement->getIntervalFormatted(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dateAsIndex=>$dateAsString): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                        ...
                        ?>
                        <td class="sub-numeric-bg editable-date"><?php echo e(number_format($totalAtDate,0)); ?></td>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <td <?php if($isAccumulatedRow): ?> style="color:white !important" <?php endif; ?> class="  sub-numeric-bg text-center total-row"><?php echo e(number_format($currentTotalAtDate,0)); ?></td>
                    </tr>
                    <?php endif; ?>
...
<script >
    ...
    table.DataTable({
            ...
            buttons: [
                 {
                    "attr": {}
                    , "text": '<i class="fa fa-pen-alt  " style="color:#366cf3;"></i>' + '<?php echo e(__("Cash Opening Balances")); ?>'
                    ...
                    , "action": function() {
                        window.location.href = "<?php echo e(route('admin.show-cash-and-banks',[$company->id,$cashFlowStatement->id,$reportType])); ?>"
                    }
                },
                ...
                , {
                    ...
                    , "action": function() {
                        ...
                        let exportFormAction = "<?php echo e(route('admin.export.cash.flow.statement.report',$company->id)); ?>";
...
<?php $__env->stopPush(); ?>
<?php /**PATH /media/salah/Software/projects/veroo/resources/views/admin/cash-flow-statement/report/view-table.blade.php ENDPATH**/ ?>