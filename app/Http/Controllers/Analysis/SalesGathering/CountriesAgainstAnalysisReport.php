        ...
        foreach ($countries as  $country) {
            if ($result == 'view') {
                    ...
                    $countries_data =collect(DB::select(DB::raw("
                        ...
                        WHERE ( company_id = '".$company->id."'AND country = '".$country."' AND date between '".$request->start_date."' and '".$request->end_date."')
                        ORDER BY id"
                ...
                $countries_data = DB::table('sales_gathering')
                    ...
                    $countries_data_quantity = DB::table('sales_gathering')
                    ->where('company_id',$company->id)
                    ->where('country', $country)
                    ->whereNotNull($type)
                    ->whereBetween('date', [$request->start_date, $request->end_date])
            ...
            foreach ($request->sales_channels as $sales_channel_key => $sales_channel) {
                ...
                $data_per_main_item = $countries_data[$sales_channel]??[];
                ...
                if (count(($data_per_main_item))>0 ) {
                    ...
                    $report_data[$country][$sales_channel][$name_of_report_item] = $data_per_main_item;
                    $interval_data = Intervals::intervals($report_data[$country][$sales_channel], $years, $request->interval);
                    $report_data[$country][$sales_channel] = $interval_data['data_intervals'][$request->interval] ?? [];

                    $report_data[$country]['Total']  = $this->finalTotal([($report_data[$country]['Total']  ?? []) ,($report_data[$country][$sales_channel][$name_of_report_item]??[]) ]);
                    $report_data[$country][$sales_channel]['Growth Rate %'] = $this->growthRate(($report_data[$country][$sales_channel][$name_of_report_item] ?? []));
                }
            }
            ...
            if($result == 'array'){
                ...
                $data_per_main_item = $countries_data_quantity[$sales_channel]??[];
                ...
                if (count(($data_per_main_item))>0 ) {
                    ...
                    $report_data_quantity[$country][$sales_channel][$name_of_report_item] = $data_per_main_item;
                    $report_data_quantity = Intervals::intervals($report_data[$country][$sales_channel], $years, $request->interval);
                    $report_data[$country][$sales_channel] = $report_data_quantity['data_intervals'][$request->interval] ?? [];

                    $report_data_quantity[$country]['Total']  = $this->finalTotal([($report_data_quantity[$country]['Total']  ?? []) ,($report_data_quantity[$country][$sales_channel][$name_of_report_item]??[]) ]);
                    $report_data_quantity[$country][$sales_channel]['Growth Rate %'] = $this->growthRate(($report_data_quantity[$country][$sales_channel][$name_of_report_item] ?? []));
                }
            }
            ...
            $final_report_total = $this->finalTotal( [($report_data[$country]['Total']??[]) , ($final_report_total??[]) ]);
            $report_data[$country]['Growth Rate %'] =  $this->growthRate(($report_data[$country]['Total'] ?? []));
            $countries_names[] = (str_replace(' ', '_', $country));
        }
    ...
    public function CountriesSalesAnalysisResult(Request $request, Company $company)
    {
        ...
        foreach ($countries as  $country) {
            ...
            if (count($countries_data)>0) {
                ...
                $years = array_unique($years);
                $report_data[$country] = $countries_data;
                $interval_data_per_item[$country] = $countries_data;
                $interval_data = Intervals::intervals($interval_data_per_item, $years, $request->interval);

                $report_data[$country] = $interval_data['data_intervals'][$request->interval][$country] ?? [];
                $growth_rate_data[$country] = $this->growthRate($report_data[$country]);
            }
        }
        ...
        $countries_names =[];
        foreach ($countries as  $country) {
            $final_report_data[$country]['Sales Values'] = ($report_data[$country]??[]);
            $final_report_data[$country]['Growth Rate %'] = ($growth_rate_data[$country]??[]);
            $countries_names[] = (str_replace( ' ','_', $country));
        }
