...
class VueDirectFactoringRevenueStreamBreakdownController extends Controller
{
	...
	public function create(Company $company , Request $request,Study $study){
		...
		return view( 'non_banking_services.vue-direct-factoring-revenue-stream-breakdown.form', $viewVars);
	}
	...
	public function store(Company $company , StoreDirectFactoringRevenueStreamRequest $request,Study $study)
	{
		...
		if($studyHasDirectFactoringBreakdowns){
			return response()->json([
				'redirectTo'=>route('create.reverse.factoring.revenue.stream.breakdown.vue',['company'=>$company->id,'study'=>$study->id])
			]);	
		}
		return response()->json([
			'redirectTo'=>route('create.direct.factoring.revenue.stream.breakdown.vue',['company'=>$company->id,'study'=>$study->id]) 
			// .'#new-funding-id'
		]);
