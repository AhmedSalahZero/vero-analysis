...
class CashFlowStatementController extends Controller
{
    ...
    public function createReport(Company $company, CashFlowStatement $cashFlowStatement)
    {
        ...
        return view('admin.cash-flow-statement.report.view', CashFlowStatement::getReportViewVars([
            'cash_flow_statement_id' => $cashFlowStatement->id, 'cashFlowStatement' => $cashFlowStatement
        ]));
    }
    ...
    public function updateReport(Company $company, Request $request)
    {
        ...
        $cashFlowStatementItemId = $request->get('cash_flow_statement_item_id');
        $cashFlowStatement = CashFlowStatement::find($cashFlowStatementId);
        $cashFlowStatementItem = $cashFlowStatement->mainItems()->wherePivot('cash_flow_statement_item_id', $cashFlowStatementItemId)->first();
        // dd($request->get('sub_item_name'));
        $cashFlowStatementItem->subItems()->wherePivot('sub_item_name', $request->get('sub_item_name'))
            ->wherePivot('cash_flow_statement_id', $cashFlowStatementId)
            ->updateExistingPivot($cashFlowStatementId, [
                'sub_item_name' => $request->get('new_sub_item_name'),
                'cash_flow_statement_item_id' => $request->get('sub_of_id'),
    ...
    public function deleteReport(Company $company, Request $request)
    {
        ...
        $cashFlowStatementItem = $cashFlowStatement->mainItems()->wherePivot('cash_flow_statement_item_id', $cashFlowStatementItemId)->first();
        // dd($request->get('sub_item_name'));
        $cashFlowStatementItem->subItems($cashFlowStatementId)->wherePivot('sub_item_name', $request->get('sub_item_name'))->detach($cashFlowStatementId);
        return response()->json([
            'status' => true,
