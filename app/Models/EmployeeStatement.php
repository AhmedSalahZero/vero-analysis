...
class EmployeeStatement extends Model  implements IHaveStatement
{
	...
	// 	->where('full_date','<=',now())
	// 	->orderByRaw('full_date asc , id asc')
	// 	->each(function($employeeStatementRow){
	// 		$letterOfGuaranteeIssuanceId = $employeeStatementRow->letter_of_guarantee_issuance_id;
	...
	// 		->where('date' ,'<=' , $employeeStatementRow->full_date)
	// 		->sum('amount');
			
	// 		DB::table('employee_statements')->where('id',$employeeStatementRow->id)
	// 		->update([
	// 			'is_active'=>1 ,
	...
	// 	 */
	// 		self::updateNextRows(self::find($employeeStatementRow->id));
