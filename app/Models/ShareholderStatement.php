...
class ShareholderStatement extends Model  implements IHaveStatement
{
	...
	// 	->where('full_date','<=',now())
	// 	->orderByRaw('full_date asc , id asc')
	// 	->each(function($shareholderStatementRow){
	// 		$letterOfGuaranteeIssuanceId = $shareholderStatementRow->letter_of_guarantee_issuance_id;
	...
	// 		->where('date' ,'<=' , $shareholderStatementRow->full_date)
	// 		->sum('amount');
			
	// 		DB::table('shareholder_statements')->where('id',$shareholderStatementRow->id)
	// 		->update([
	// 			'is_active'=>1 ,
	...
	// 	 */
	// 		self::updateNextRows(self::find($shareholderStatementRow->id));
