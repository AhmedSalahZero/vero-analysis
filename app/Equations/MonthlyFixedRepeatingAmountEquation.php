...
{
	public function calculate(float $amount,int $startDateAsIndex,int $endDateAsIndex,string $increaseInterval, $increaseRate,bool $isDeductible,float $vatRate,float $withholdRate,$dateIndexWithYearIndex = []):array 
	{
		$resultWithoutVat = [];
		$resultWithVat = [];
		$resultVat = [];
		...
		for($currentStartDateAsIndex ; $currentStartDateAsIndex <= $endDateAsIndex ; $currentStartDateAsIndex++ ){
			...
			$increaseRate = is_array($increaseRate) ? $increaseRate[$dateIndexWithYearIndex[$currentStartDateAsIndex]] : $increaseRate ;
