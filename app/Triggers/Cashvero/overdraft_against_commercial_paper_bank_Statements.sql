				...
				begin
					...
					--	declare _current_customer_id integer default 0 ;
						...
						insert into debugging (message) values (concat('deposit date',_deposit_date));
						insert into debugging (message) values (concat('money_received_id',_money_received_id));
						...
						-- insert into debugging (message) values (concat('no cheques at date',_counter,'full date',_full_date));
				...
				delimiter ; 
				drop procedure if exists reverse_overdraft_against_commercial_by_specific_debit ;
				delimiter // 
				create procedure reverse_overdraft_against_commercial_by_specific_debit(in _start_update_from_date_time date  , in _overdraft_against_commercial_paper_id integer , in _current_debit decimal(14,2) )
				begin 
				...
				-- هنجيب كل السحوبات اللي تاريخها اكبر من تاريخ الاغلاق لان اللي تاريخها اصغر من او يساوي تاريخ الاغلاق مش هنقدر نيجي يمها
					...
					repeat 
						select id , settlement_amount into _overdraft_against_commercial_paper_withdrawal_id , _current_settlement from overdraft_against_commercial_paper_withdrawals where 
					-- due_date > _start_update_from_date_time and
					overdraft_against_commercial_paper_id = _overdraft_against_commercial_paper_id  and settlement_amount > 0 order by due_date desc , id desc limit 1 ;
					set _settlement_amount = IIF(_current_settlement >_current_debit, _current_debit, _current_settlement) ;
					update overdraft_against_commercial_paper_withdrawals set net_balance = net_balance + _settlement_amount , settlement_amount = settlement_amount - _settlement_amount where id =  _overdraft_against_commercial_paper_withdrawal_id ;
			...
			--				select start_settlement_from_bank_statement_date into _last_bank_statement_date_to_start_settlement_from from overdraft_against_commercial_papers where id = new.overdraft_against_commercial_paper_id ; 
					...
					-- if(new.full_date > _start_update_from_date_time ) then 		 -- دي في حالة لو انت اشتغلت علي موضوع ال closing date 
						--	delete from overdraft_against_commercial_paper_settlements where overdraft_against_commercial_paper_bank_statement_id = _last_id;
				...
				--			call reverse_overdraft_against_commercial_by_specific_debit(_start_update_from_date_time,new.overdraft_against_commercial_paper_id,_current_debit);	
				--		else 
				--			call resettlement_overdraft_against_commercial_paper_from(_start_update_from_date_time,new.overdraft_against_commercial_paper_id,new.company_id);
				--		end if;
					end if;
				...
				delimiter // 
				create  trigger insert_into_overdraft_withdrawal_after_insert after insert on `overdraft_against_commercial_paper_bank_statements` for each row 
				begin 
