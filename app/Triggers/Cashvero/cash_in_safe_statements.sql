...
begin 
	...
	declare _previous_date date default null ;
		...
		if new.id then 
		-- في حاله التعديل
		select date,end_balance into _previous_date, _last_end_balance  from cash_in_safe_statements where company_id = new.company_id  and id < new.id order by id desc limit 1 ;
		set _count_all_rows = 1 ;
		else
		-- ف
		select date , end_balance  into _previous_date,_last_end_balance  from cash_in_safe_statements where company_id = new.company_id  order by id desc limit 1 ;
		select  count(*) into _count_all_rows from cash_in_safe_statements where company_id = new.company_id  order by id desc limit 1 ;
		end if;
