				drop trigger if exists refresh_calculation_before_insert_fully_secured_overdraft ;
				delimiter // 
				create  trigger refresh_calculation_before_insert_fully_secured_overdraft before insert on `fully_secured_overdraft_bank_statements` for each row 
				begin 
				...
				delimiter ;
				drop trigger if exists  refresh_calculation_before_update_fully_secured_overdraft ;
				drop trigger if exists  calculate_fully_secured_overdraft_limit ;
			...
			--				select start_settlement_from_bank_statement_date into _last_bank_statement_date_to_start_settlement_from from fully_secured_overdrafts where id = new.fully_secured_overdraft_id ; 
				...
				begin 
					...
					set _due_date = if(_type = 'outstanding_balance' , _date_for_settlement ,ADDDATE(_date_for_settlement,_fully_secured_overdraft_to_be_settled_after));
					set _fully_secured_overdraft_to_be_settled_after = ifnull(_fully_secured_overdraft_to_be_settled_after , 0) ; 
				...
				begin 
					...
					if new.is_credit > 0 then
						call start_settlement_process_fully_secured_overdraft(new.type,new.id , new.fully_secured_overdraft_id , new.debit  , new.credit , new.company_id ,_date_for_settlement);
					end if;
				end //
