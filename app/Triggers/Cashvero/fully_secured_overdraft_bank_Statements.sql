		drop trigger if exists refresh_calculation_before_insert_fully_secured ;
		delimiter // 
		create  trigger refresh_calculation_before_insert_fully_secured before insert on `fully_secured_overdraft_bank_statements` for each row 
		begin 
		...
		delimiter ;
		drop trigger if exists  refresh_calculation_before_update_fully_secured ;
		drop procedure if exists resettlement_fully_secured_overdraft_from ;
		delimiter // 
		create procedure resettlement_fully_secured_overdraft_from(in _start_update_from_date_time date , in _fully_secured_overdraft_id integer , in _current_company_id integer  )
		begin 
		...
		create trigger refresh_calculation_before_update_fully_secured before update on `fully_secured_overdraft_bank_statements` for each row 
		begin 
	...
	--				select start_settlement_from_bank_statement_date into _last_bank_statement_date_to_start_settlement_from from fully_secured_overdrafts where id = new.fully_secured_overdraft_id ; 
			...
			if(_last_bank_statement_date_to_start_settlement_from = new.full_date) then 		
				...
				select sum(debit) into _current_debit from fully_secured_overdraft_bank_statements where fully_secured_overdraft_id = new.fully_secured_overdraft_id and is_debit > 0    ;
				select sum(settlement_amount) into _total_settlements from fully_secured_overdraft_withdrawals where fully_secured_overdraft_id =  new.fully_secured_overdraft_id ;
				set _current_debit = _current_debit - _total_settlements ;
		...
		begin 
			...
			if _fully_secured_overdraft_to_be_settled_after > 0 then  -- في الحاله دي هنضيف القيم في جداول fully_secured_overdraft_settlements + fully_secured_overdraft_withdrawals
				...
				-- get first item need to be settled  هنجيب اول عنصر في المسحوبات محتاج يتعمله تسديد .. اللي هو النت بالانس بتاعه اكبر من الصفر
					...
					-- insert into fully_secured_overdraft_settlements (fully_secured_overdraft_bank_statement_id,fully_secured_overdraft_withdrawal_id,fully_secured_overdraft_id , company_id   , settlement_amount,created_at) values(0,_fully_secured_overdraft_withdrawal_id,_fully_secured_overdraft_id,_company_id,_current_settlement_amount,CURRENT_TIMESTAMP);
