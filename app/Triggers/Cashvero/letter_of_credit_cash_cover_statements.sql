drop trigger if exists before_insert_lc_cash_cover_statements ;
delimiter // 
create  trigger before_insert_lc_cash_cover_statements before insert on `letter_of_credit_cash_cover_statements` for each row 
begin 
...
delimiter ; 
drop trigger if exists before_update_lc_cash_cover_statements ;
delimiter // 
create  trigger before_update_lc_cash_cover_statements before update on `letter_of_credit_cash_cover_statements` for each row 
begin 
		declare _last_end_balance decimal(14,2) default 0 ;
		declare _previous_date date default null ;
		declare _count_all_rows integer default 0 ; 
