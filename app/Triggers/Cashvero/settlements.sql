...
BEGIN
	...
	select sum(settlement_amount) into _total_down_payment_settlement from settlements where money_received_id = new.money_received_id and is_from_down_payment = 1 ;
	...
	update `customer_invoices` set withhold_amount = _withhold_amount  where id = new.invoice_id   ;
	 ...
	 update `down_payment_settlements` set total_down_payment_settlement = _total_down_payment_settlement where money_received_id = new.money_received_id;
...
BEGIN
	...
	select sum(settlement_amount) into _total_down_payment_settlement from settlements where money_received_id = old.money_received_id and is_from_down_payment = 1 ;
	...
	update `down_payment_settlements` set total_down_payment_settlement = _total_down_payment_settlement where money_received_id = old.money_received_id;
END//
