...
begin 
	...
	declare _previous_date date default null ;
		...
		select date , end_balance  into _previous_date,_last_end_balance  from letter_of_guarantee_statements where company_id = new.company_id and currency = new.currency and lg_facility_id = new.lg_facility_id and cd_or_td_id = new.cd_or_td_id and financial_institution_id = new.financial_institution_id and source = new.source and lg_type = new.lg_type  and  full_date < new.full_date   order by full_date desc , id desc limit 1 ;
		select  count(*) into _count_all_rows from letter_of_guarantee_statements where company_id = new.company_id and currency = new.currency and lg_facility_id = new.lg_facility_id and cd_or_td_id = new.cd_or_td_id and lg_type = new.lg_type and financial_institution_id = new.financial_institution_id and source = new.source and full_date < new.full_date   order by full_date desc , id desc limit 1 ;
...
begin 
		...
		declare _previous_date date default null ;
		declare _count_all_rows integer default 0 ; 
		select date,end_balance into _previous_date, _last_end_balance  from letter_of_guarantee_statements where company_id = new.company_id and lg_facility_id = new.lg_facility_id and cd_or_td_id = new.cd_or_td_id and cd_or_td_id = new.cd_or_td_id and financial_institution_id = new.financial_institution_id and source = new.source and lg_type = new.lg_type and currency = new.currency and full_date < new.full_date order by full_date desc , id desc limit 1 ;
