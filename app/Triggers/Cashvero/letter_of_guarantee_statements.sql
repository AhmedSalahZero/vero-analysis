...
begin 
	...
	declare _previous_date date default null ;
	 ...
	 insert into debugging (message) values (concat('from create new beg',new.beginning_balance , 'debit',new.debit,'credit',new.credit));
...
begin 
	...
	-- خده كوبي وبيست وحطة هنا
	 ...
	 insert into debugging (message) values (concat('from update new beg',new.beginning_balance , 'debit',new.debit,'credit',new.credit));
	set new.end_balance = new.beginning_balance + new.debit - new.credit ; 
